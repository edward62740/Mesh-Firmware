...
void mesh_response_data(uint8_t pid,uint16_t dest,uint8_t * data,uint8_t size)
{
    message_t msg;
...
void mesh_tx_iaq(int32_t temp,uint32_t hum,uint32_t press,uint32_t light,uint32_t iaq)
{
    uint8_t data[20];
    data[0] = 0xFF & (uint8_t)(temp >> 24);
    data[1] = 0xFF & (uint8_t)(temp >> 16);
    ...
    data[14]= 0xFF & (uint8_t)(light >> 8);
    data[15]= 0xFF & (uint8_t)(light );
    data[16]= 0xFF & (uint8_t)(iaq >> 24);
    data[17]= 0xFF & (uint8_t)(iaq >> 16);
    data[18]= 0xFF & (uint8_t)(iaq >> 8);
    data[19]= 0xFF & (uint8_t)(iaq );
    mesh_bcast_data(MESH_PID_IAQ,data,20);
}

void mesh_tx_range(uint16_t distance, uint16_t cal_point, uint16_t cal_tolerance, uint8_t range_mode)
{
    uint8_t data[7];
    ...
    mesh_bcast_data(MESH_PID_RANGE,data,7);
}
...
int rx_env(char * p_msg,uint8_t*data,uint8_t size)
{
    ...
    if(size != 16)
    {
            return sprintf(p_msg,"size not 14 but:%u",size);
    }
    else
    {
...
int rx_iaq(char * p_msg,uint8_t*data,uint8_t size)
{
    char * p_start = p_msg;
    if(size != 20)
    {
    ...
    else
    {
        ...
        p_msg += sprintf(p_msg,";iaq:%.02f",f_iaq);
...
int rx_motion(char * p_msg,uint8_t*data,uint8_t size)
{
    if(size == 26)
    {
        ...
        return sprintf(p_msg,"x:%.03f;y:%.03f;z:%.03f;shockdur:%.03f;shockcount:%.i;stepcount:%.i;sensetemp:%.i",float_x, float_y, float_z, float_shock_dur, shocks, steps, temp);
    }
    else
    {
        return sprintf(p_msg,"size not 3 but:%d",size);
    }
}
...
int rx_range(char * p_msg,uint8_t*data,uint8_t size)
{
    if(size == 7)
    {
        ...
        return sprintf(p_msg,"distance:%.i;cal:%.i;tol:%.i;mode:%.i",distance_mm, cal_point_mm, cal_tolerance_mm, range_mode_int);
    }
    else
    {
        return sprintf(p_msg,"size not 3 but:%d",size);
...
void mesh_parse(message_t* msg,char * p_msg)
{
    ...
    switch(msg->pid)
    {
        ...
        case MESH_PID_IAQ:
            {
                p_msg += rx_iaq(p_msg,msg->payload,msg->payload_length);
            }
            break;
